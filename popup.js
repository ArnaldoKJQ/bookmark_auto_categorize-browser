...
enabledToggle.addEventListener("change", () => {
  ...
  if (enabled) {
    chrome.runtime.sendMessage({ type: "TOGGLE_ON" }, response => {
      if (response && response.ok) updateInboxStatus();
    });
  } else {
    chrome.runtime.sendMessage({ type: "TOGGLE_OFF" }, () => updateInboxStatus());
  }
});
...
function renderLog(log) {
  ...
  container.innerHTML = log.map(entry => {
    ...
    const shortUrl = (() => { try { return new URL(entry.url).hostname; } catch { return entry.url; } })();
    return `
      <div class="log-item">
